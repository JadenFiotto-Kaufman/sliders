...
from lora import (DEFAULT_TARGET_REPLACE, UNET_TARGET_REPLACE_MODULE_CONV,
                  LoRANetwork)
...
def sorted_nicely(l):
    convert = lambda text: (
        float(text) if text.replace("-", "").replace(".", "").isdigit() else text
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rank", help="rank of the LoRA", type=int, required=False, default=4
    )
    parser.add_argument(
        "--start_noise",
        help="what time stamp to flip to edited model",
        type=int,
        required=False,
        default=750,
    )
    ...
    scales = [-2, -1, 0, 1, 2]
    scales = [-1, -0.5, 0, 0.5, 1]
    scales = [-2]
    df = pd.read_csv(csv_path)

    for scale in scales:
        os.makedirs(
            f"{save_path}/{os.path.basename(lora_weight)}/{scale}", exist_ok=True
        )
    ...
    for idx, prompt in enumerate(prompts):
        ...
        if not (case_number >= from_case and case_number <= till_case):
            continue
        if os.path.exists(
            f"{save_path}/{os.path.basename(lora_weight)}/{scale}/{case_number}_{idx}.png"
        ):
            continue
        print(prompt, seed)
        for scale in scales:
